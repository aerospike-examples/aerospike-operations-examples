...
try:
    ...
    client.operate(key, [listops.list_append_items("l", [1, 2])])
    ...
    client.operate(key, [listops.list_append_items("l", [1, 3], policy)])
    ...
    ret = client.operate(key, [listops.list_append_items("l", [4, 5], ctx=ctx)])
    key, metadata, bins = client.get(key)
